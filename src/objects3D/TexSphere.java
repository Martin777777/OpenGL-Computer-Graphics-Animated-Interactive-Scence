...
import GraphicsObjects.Calculation;
		...
		float s,t; // texture coordinates
		   ...
		   float inctheta = (float) ((2.0f*Math.PI )/ nSlices );
		    ...
		    for(float theta=(float) -Math.PI; theta<Math.PI; theta+=inctheta)
		    {
		        ...
		        {
		            x = (float) (Calculation.cos(phi)* Math.cos(theta)*radius);
		            y =  (float) (Calculation.cos(phi)* Math.sin(theta)*radius);
		            z =  (float) (Calculation.sin(phi)*radius);
		            ...
		            x = (float) (Calculation.cos(phi)*Math.cos(theta+inctheta)*radius);
		            y = (float) (Calculation.cos(phi)*Math.sin(theta+inctheta)*radius);
		            z = (float) (Calculation.sin(phi)*radius);
		         ...
		         GL11.glNormal3f(x,y,z); //Mistake in previous version fixed ( abey 11/1/2018)
		            ...
		            x = (float) (Calculation.cos(phi+incphi)*Calculation.cos(theta+inctheta)*radius);
		            y = (float) (Calculation.cos(phi+incphi)*Calculation.sin(theta+inctheta)*radius);
		            z = (float) (Calculation.sin(phi+incphi)*radius);
		            ...
		            x = (float) (Calculation.cos(phi+incphi)*Calculation.cos(theta)*radius);
		            y = (float) (Calculation.cos(phi+incphi)*Calculation.sin(theta)*radius);
		            z = (float) (Calculation.sin(phi+incphi)*radius);
		            t = (float) ((((float)phi+incphi)/(float)Math.PI)+0.5f);
